...
namespace code.Services
{
	public class TemplateManagerService
	{
		...
		public async void AddTemplate(TrainTemplate tmp)
		{
			...
			parameters.Add(new NpgsqlParameter("p1", tmp.Name));
			parameters.Add(new NpgsqlParameter("p2", tmp.Destination));
			parameters.Add(new NpgsqlParameter("p7", tmp.MaxLength));
		...
		public async void UpdateTemplate(TrainTemplate tmp)
		{
			...
			List<NpgsqlParameter> parameters = new List<NpgsqlParameter>();
			parameters.Add(new NpgsqlParameter("p1", tmp.Id));
			parameters.Add(new NpgsqlParameter("p2", tmp.Name));
			parameters.Add(new NpgsqlParameter("p3", tmp.Destination));
			parameters.Add(new NpgsqlParameter("p4", tmp.MaxLength));
